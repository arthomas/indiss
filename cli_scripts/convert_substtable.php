 ...
 */
    ...
    function write_out($output, $date) {
        for ($i = 0; $i < ($count = count($output)); $i++) {
            file_put_contents($date . "_$i.html", sprintf($output[$i], $count));
        }
    }
    ...
    $thead = "";
    $thead .= '    <table border="0" cellspacing="0" cellpadding="0">' . "\n";
    $thead .= '        <tbody>' . "\n";
    ...
    $thead .= '            <tr><th colspan="10">Datum: %s, Seite %d/%%d</th></tr>' . "\n";
    ...
    foreach ($table as $date => $thisDate) {
        if ( $pages > 0 ) {
            ...
            write_out($output, $date);
            $pages = 0;
            unset($output);
        }
        if ( empty( $output[$pages] ) ) {
            $output[$pages] = $head . sprintf($thead, $date, $pages + 1);
            $lines = $startlines;
        }
